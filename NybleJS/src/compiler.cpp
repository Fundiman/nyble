...
void Compiler::emit(OpCode op) { chunk->emit(op, 0); }
void Compiler::emitByte(uint8_t b) { chunk->emitByte(b, 0); }
void Compiler::emitShort(uint16_t s) { chunk->emitShort(s, 0); }
...
void Compiler::patchJump(size_t offset) {
    chunk->patch(offset, (uint16_t)(chunk->count() - offset - 2));
}

void Compiler::emitLoop(size_t start) {
    emit(OpCode::LOOP);
    emitShort((uint16_t)(chunk->count() - start + 2));
}

size_t Compiler::makeString(const std::string& s) { return chunk->addStr(s); }
size_t Compiler::makeNum(double n) { return chunk->addNum(n); }

void Compiler::enterScope() { scopeDepth++; emit(OpCode::SCOPE_ENTER); }
void Compiler::exitScope() { scopeDepth--; emit(OpCode::SCOPE_EXIT); }
...
void Compiler::compileExprStmt(ExprStmtNode* stmt) {
    compileExpr(stmt->expr.get());
    emit(OpCode::POP);
}

void Compiler::compileVarDecl(VarDeclNode* decl) {
...
void Compiler::compileFunDecl(FunDeclNode* decl) {
...
void Compiler::compileIf(IfNode* stmt) {
...
void Compiler::compileWhile(WhileNode* stmt) {
...
void Compiler::compileDoWhile(DoWhileNode* stmt) {
    size_t old = loopStart; bool oldH = hasLoop;
    loopStart = chunk->count(); hasLoop = true;
    compileStmt(stmt->body.get());
    compileExpr(stmt->cond.get());
...
void Compiler::compileFor(ForNode* stmt) {
    enterScope();
...
void Compiler::compileReturn(ReturnNode* stmt) {
    if (stmt->value) compileExpr(stmt->value.get());
    else emit(OpCode::PUSH_UNDEFINED);
    emit(OpCode::RETURN);
}

void Compiler::compileSwitch(SwitchNode* stmt) {
    compileExpr(stmt->expr.get());
    emit(OpCode::DUP);
...
void Compiler::compileThrow(ThrowNode* stmt) {
    if (stmt->value) compileExpr(stmt->value.get());
    else emit(OpCode::PUSH_UNDEFINED);
    emit(OpCode::THROW);
}

void Compiler::compileTry(TryNode* stmt) {
...
void Compiler::compileBinary(BinaryExprNode* expr) {
    if (expr->op == "&&") {
...
void Compiler::compileUnary(UnaryExprNode* expr) {
...
void Compiler::compileMember(MemberExprNode* expr) {
...
void Compiler::compileIdentifier(IdentifierNode* expr) {
    size_t s = makeString(expr->name);
    emit(OpCode::LOAD);
    emitShort((uint16_t)s);
}

void Compiler::compileLiteral(LiteralNode* expr) {
    auto& v = expr->value;
    switch (v.type) {
...
void Compiler::compileArray(ArrayLitNode* expr) {
...
void Compiler::compileObject(ObjectLitNode* expr) {
...
void Compiler::compileAssign(AssignNode* expr) {
...
void Compiler::compileConditional(ConditionalNode* expr) {
...
void Compiler::compileNew(NewExprNode* expr) {
    compileExpr(expr->callee.get());
    for (auto& a : expr->args) compileExpr(a.get());
    emit(OpCode::NEW);
    emitShort((uint16_t)expr->args.size());
...
void Compiler::compileArrow(ArrowFuncNode* expr) {
    auto funcChunk = new BytecodeChunk();
